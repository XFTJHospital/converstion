#!/usr/bin/env python3
"""Streamlit app for the clean 8-predictor HCC EHM calculator.

This app expects the clean lower_snake_case bundles created by:
    python train_model_lung_8var.py --csv lung.csv --out-dir .
    python train_model_bone_8var.py --csv bone.csv --out-dir .

Do not mix this app with old bundles that store feature names such as
AFP_ge400, Tumor_Size_ge5, or Child_Pugh_BC.
"""
...
# All model constants are defined here so Streamlit Cloud does not need a separate
# model_config.py file. Keep these names synchronized with the clean bundle pkl files.
...
LUNG_CUTOFF = 0.487
BONE_CUTOFF = 0.490
SCHEMA_VERSION = "hcc_ehm_8predictor_lower_snake_case_v3"

ENFORCE_MONOTONIC_HORIZON_SCORES = True
LOGO_PATH = Path("11814654.jpg")
...
def load_bundle(path: str) -> dict:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"{path} not found. Please upload app.py, lung_model_bundle.pkl, bone_model_bundle.pkl, "
            "11814654.jpg, and requirements.txt in the same Streamlit app folder."
        )
    bundle = joblib.load(bundle_path)
    required_keys = {"schema_version", "features", "horizons", "models", "cutoff"}
    missing = sorted(required_keys - set(bundle.keys()))
    if missing:
        raise ValueError(f"{path} is not a valid bundle. Missing keys: {missing}")
    return bundle


def validate_bundle(bundle: dict, expected_features: list[str], expected_cutoff: float, model_name: str) -> None:
    if bundle.get("schema_version") != SCHEMA_VERSION:
        raise ValueError(
            f"{model_name} bundle schema mismatch. Expected {SCHEMA_VERSION}, "
            f"but found {bundle.get('schema_version')!r}. Retrain using the clean scripts."
        )
    actual_features = list(bundle.get("features", []))
    if actual_features != expected_features:
        raise ValueError(
            f"{model_name} feature mismatch. Expected {expected_features}, but bundle contains {actual_features}. "
            "Please use the newly generated clean lower_snake_case bundle."
        )
    actual_horizons = list(bundle.get("horizons", []))
    if actual_horizons != HORIZONS:
        raise ValueError(f"{model_name} horizon mismatch. Expected {HORIZONS}, but found {actual_horizons}.")
    if abs(float(bundle.get("cutoff")) - float(expected_cutoff)) > 1e-12:
        raise ValueError(f"{model_name} cutoff mismatch. Expected {expected_cutoff}, but found {bundle.get('cutoff')}.")
...
def predict_scores(bundle: dict, feature_values: dict[str, int]) -> list[tuple[str, str, float]]:
    ...
    X = pd.DataFrame([{f: int(feature_values[f]) for f in features}], columns=features)
    scores: list[float] = []
...
def yes_no_radio(label: str, default: str = "No") -> str:
    options = ["No", "Yes"]
    return st.sidebar.radio(label, options, index=options.index(default))


def display_result_card(label: str, risk: str, score: float) -> None:
...
def lung_page() -> None:
    ...
    try:
        ...
        validate_bundle(bundle, LUNG_FEATURES, LUNG_CUTOFF, "LM")
    ...
    if st.session_state["lung_predicted"] and st.session_state["lung_results"]:
        ...
        for horizon, risk, score in results:
            display_result_card(horizon, risk, score)
...
def bone_page() -> None:
    ...
    try:
        ...
        validate_bundle(bundle, BONE_FEATURES, BONE_CUTOFF, "BM")
    except (FileNotFoundError, ValueError) as exc:
        st.error(str(exc))
        return
...
if st.session_state.page == "home":
    ...
    with title_col:
        st.markdown(
            "<h1 style='color:#2a9d8f;margin-bottom:0;'>Tongji HCC Extrahepatic Metastasis Prediction Platform</h1>",
            unsafe_allow_html=True,
        )
        st.markdown(
            """
            ...
            Results are reported as model-derived risk scores with high/low-risk
            categories generated by the locked operating rule.
            """
        )
