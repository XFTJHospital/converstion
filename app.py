...
- Feature names are unified as lower_snake_case and match the model bundles.
"""
...
def load_bundle(path: str) -> dict:
    ...
    if "features" not in bundle or "models" not in bundle or "cutoff" not in bundle:
...
def validate_bundle_features(bundle: dict, expected_features: list[str], model_name: str) -> None:
    actual = list(bundle.get("features", []))
    if actual != expected_features:
        raise ValueError(
            f"{model_name} feature mismatch. Expected {expected_features}, but bundle contains {actual}."
        )
